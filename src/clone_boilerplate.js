...
function cloneBoilerplate(projectName) {
  ...
  function basic(options, destinationFolder) {
    ...
    fileUtils.copy(__dirname + '/../boilerplates/basic',
                   destinationFolder,
                   processFile.bind(null, projectName));

    fileUtils.rename(destinationFolder + '/gitignore',
                     destinationFolder + '/.gitignore');

    fileUtils.rename(destinationFolder + '/eslintrc',
                     destinationFolder + '/.eslintrc');

    fileUtils.rename(destinationFolder + '/npmignore',
                     destinationFolder + '/.npmignore');

    console.log('  - Done\n');
  }

  function cssModules(destinationFolder) {
    fileUtils.copy(__dirname + '/../boilerplates/css_modules',
                   destinationFolder,
                   processFile.bind(null, projectName));
  }
}
